...
一个集成多种数据采集功能的自动化工具集，包含58同城招聘信息爬虫和学校信息自动化采集两大核心功能。

**🎯 项目亮点**：
- **58同城爬虫**：已成功验证，单次运行38.53分钟抓取1105个有效职位，平均2.09秒/职位，数据完整性100%
- **学校信息采集**：支持142所高校的专业设置和百科信息自动化采集，具备智能日志系统
...
│   └── log/                              # 日志文件目录（运行时自动创建）
├── school/                               # 学校信息采集功能目录
│   ├── browser_automation.py            # 浏览器自动化脚本
│   ├── 学校.txt                          # 支持的学校名单（142所高校）
│   └── logs/                             # 自动化日志目录
├── other/                                # 辅助工具目录
